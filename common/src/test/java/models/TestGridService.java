...
public class TestGridService {
    ...
    void testSetStartTiles() {
        ...
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.setStartTiles(new int[]{-1});
        });
        // Check valid indices
        assertDoesNotThrow(() -> {
    ...
    void testSetCharacterTo() {
        ...
        assertDoesNotThrow(() -> {
            gridService.setCharacterTo(vec5valid, character1);
        });
        assertEquals(vec5valid, character1.getPosition());
        // Check invalid position (out of bounds)
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.setCharacterTo(vec1invalid, character1);
        });
        ...
        assertThrows(DestinationAlreadyOccupiedException.class, () -> {
            gridService.setCharacterTo(vec5valid, character2);
        });
    ...
    void testSwapCharacters() {
        ...
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.swapCharacters(vec1invalid, vec2invalid);
        });
        // Check first tile invalid
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.swapCharacters(vec1invalid, vec5valid);
        });
        // Check first tile empty
        assertThrows(NoCharacterFoundException.class, () -> {
            gridService.swapCharacters(vec5valid, vec5valid);
        });
        ...
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.swapCharacters(vec5valid, vec1invalid);
        });
        // Check first tile valid second empty
        assertThrows(NoCharacterFoundException.class, () -> {
            gridService.swapCharacters(vec5valid, vec6valid);
        });

        assertDoesNotThrow(() -> {
            gridService.setCharacterTo(vec6valid, character2);
        });
        ...
        assertDoesNotThrow(() -> {
            assertEquals(character1, gridService.getCharacterAt(vec6valid));
        });
        assertEquals(character1.getPosition(), vec6valid);
        assertDoesNotThrow(() -> {
            assertEquals(character2, gridService.getCharacterAt(vec5valid));
        });
    ...
    void testGetCharacterFrom() {
        // Check invalid position
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.getCharacterAt(vec1invalid);
        });
        // Check empty position
        assertDoesNotThrow(() -> {
            assertNull(gridService.getCharacterAt(vec5valid));
        });
        ...
        // Check valid position
        assertDoesNotThrow(() -> {
            gridService.getCharacterAt(vec5valid);
        });
    }
    ...
    void testRemoveCharacterFrom() {
        // Check invalid position
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.removeCharacterFrom(vec1invalid);
        });
        // Check empty position
        assertThrows(NoCharacterFoundException.class, () -> {
            gridService.removeCharacterFrom(vec5valid);
        });
    ...
    void testMoveCharacter() {
        ...
        assertThrows(DestinationInvalidException.class, () -> {
            gridService.moveCharacter(vec1invalid, vec2invalid);
        });
        // Check from valid to invalid
        assertThrows(NoCharacterFoundException.class, () -> {
            gridService.moveCharacter(vec5valid, vec1invalid);
        });
        ...
        assertThrows(DestinationAlreadyOccupiedException.class, () -> {
            gridService.moveCharacter(vec5valid, vec6valid);
        });
        assertNotEquals(character1.getPosition(), vec6valid);
        // Check from valid occupied to valid empty
        assertDoesNotThrow(() -> {
            gridService.moveCharacter(vec5valid, vec7valid);
        });
